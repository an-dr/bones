...
navigate, deliver extension JSON, update native state once per frame, and
return queued page messages or native close events from `drain_events`. The
wry implementation is feature-gated so this crate and headless consumers can
build without webview system libraries.
...
webviews that track the resizable window's live client area. Pages send opaque
...
An `Html` panel is served over the `bones://` custom protocol rather than set as
a document string, so the page loads at a real origin and can use storage and
anything else subject to a same-origin check. A `Url` panel navigates as given.
