...
Opens one 800x600 window (`core/platform`) and feeds keyboard events onto
`input/*`. Drop `.wasm` extensions into an `extensions/` directory next to
where you run it; `cargo run -p app` (or the built binary) discovers and
runs them.
