use crate::{
    ...
    shared_or_exe_relative, shared_or_exe_relative_from, verify_checksum, AppIdentity, Fetch,
    Manifest,
};
...
fn download_asset_verified_accepts_a_matching_checksum() {
    ...
    assert_eq!(
        download_asset_verified(&backend, &manifest).unwrap(),
        payload
    );
}
...
fn shared_or_exe_relative_from_steps_up_when_running_from_a_version_folder() {
    ...
    // plain build directory too, not only a real ~/.myapp/app install.
    let dir = tempfile::tempdir().unwrap();
