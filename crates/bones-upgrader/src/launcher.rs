...
use crate::{
    current_version_dir, launcher_version, previous_version_dir, remove_version_dir, AppIdentity,
};
...
/// created with.
///
/// A redirect wins over the console. `myapp --version > versions.txt` has
...
mod tests {
    ...
    fn rollback_refuses_to_leave_the_entry_point_with_nothing_to_start() {
        ...
        assert!(
            dir.path().join("1.3.0").exists(),
            "the only version must survive"
        );
    }
