...
/// The one capability this crate needs from its host: retrieving a URL.
///
/// Narrower than the OS surface a desktop application usually has, and
/// deliberately so -- self-update is otherwise pure filesystem and version
/// arithmetic, and a crate that asked for clipboards and file pickers to do it
/// could not be reused by a host that has neither. A test satisfies this with a
/// map of canned responses.
pub trait Fetch {
    /// Retrieves `url`, returning the body as `"{content-type};base64,{data}"`.
    ///
    /// `Ok(None)` means the URL held nothing -- a manifest that has not been
    /// published yet is a normal answer, not a failure. Network and protocol
    /// errors are `Err`.
    fn fetch_url(&self, url: &str) -> Result<Option<String>, String>;
}

/// Splits a [`Fetch::fetch_url`] body into its content type and decoded bytes.
pub fn decode_fetch_result(value: &str) -> Result<(String, Vec<u8>), String> {
    let (content_type, encoded) = value.split_once(";base64,").ok_or_else(|| {
        String::from("fetch_url result is not in the expected content-type;base64,data form")
    })?;
...
pub fn is_newer(current: &str, candidate: &str) -> bool {
    match (
        versions::parse_version_segments(current),
        versions::parse_version_segments(candidate),
    ) {
...
pub fn fetch_manifest(backend: &dyn Fetch, url: &str) -> Result<Manifest, String> {
    let fetched = backend
        .fetch_url(url)?
        .ok_or_else(|| format!("no manifest found at {url}"))?;
    let (_, bytes) = decode_fetch_result(&fetched)?;
...
pub fn download_asset_verified(
    backend: &dyn Fetch,
    manifest: &Manifest,
    ...
    if let Some(expected) = &manifest.sha256 {
        if !verify_checksum(&bytes, expected) {
            return Err(String::from(
                "downloaded asset does not match the manifest's sha256",
            ));
        }
    }
    Ok(bytes)
...
/// otherwise `~/.myapp/app`. Compared against the running process's own
...
/// location -- i.e. this process is the app executable running from a version
/// folder the launcher picked, rather than a dev build or an
...
pub fn is_installed_version_dir(identity: &AppIdentity, dir: &std::path::Path) -> bool {
    ...
    let Some(install_dir) = default_install_dir(identity).and_then(|d| d.canonicalize().ok())
    else {
        return false;
    };
...
/// shape of an installed tree, so a dev
...
pub(crate) fn is_version_folder(identity: &AppIdentity, dir: &std::path::Path) -> bool {
    ...
    dir.parent()
        .is_some_and(|parent| parent.join(identity.launcher_exe()).is_file())
}
