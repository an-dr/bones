...
`Module` gains `filter_event`, an optional hook taking the raw platform event and returning whether it claims it, defaulting to claiming nothing. The frame loop offers each event to every module and stops at the first claim, naming none of them. It offers them in **reverse registration order**: the module registered last is asked first.
...
**Registration order means one thing with two directions**, and getting this backwards is easy. The render and present phases walk it forward, so a module registered later draws *above* one registered earlier. Input therefore has to walk it backward, so the module that drew last is offered the event first. Reading both in the same direction would hand the first look to whatever is drawn furthest underneath — in the shipped composition, the renderer — which inverts [ADR-008](ADR-008-layered-input-focus.md) outright. The two are the same statement about layering read from opposite ends.

Reversing the walk rather than adding a priority field keeps the rule inspectable from the builder call site. Where a module sits in the composition is the single fact that decides both where it draws and when it is offered input; there is nothing else to set, and nothing that can disagree with itself. For the shipped composition the reversal produces ADR-008's order exactly: registration runs renderer, ui, web, injected modules, so reversed, the offer reaches web above ui above the gfx scene, with anything an embedder injects sitting above all three because it was composed after them.
...
- **An explicit input-priority number on `Module`.** Each module declares where it sits for input, independent of registration. More flexible, and it would let input order differ from draw order deliberately. Rejected: nothing has a reason to set it to anything but the reverse of composition order, so it would be a field every module author must think about to reproduce the only sensible default — and two orders that can disagree are two orders that eventually will.
- **A separate registration list for input.** `.overlay(...)` alongside `.module(...)`, with its own order. Rejected for the same reason plus a worse one: it splits one composition into two lists that must be kept consistent, and an embedder who registers in only one gets a module that draws but cannot be clicked, or the reverse.
