...
and synchronous send/lifecycle can all proceed in parallel from here —
platform, renderer, and watchdog (their shared prerequisites) are done.
...
| 2 | ui module (egui) + `ui/*` vocabulary | The "notes" example ([examples/egui-app.md](examples/egui-app.md)) runs |
| 3 | web module (wry) + `web/*` vocabulary | The "dashboard" example ([examples/web-app.md](examples/web-app.md)) runs |
| 4 | Full builder API: custom native-module injection, embedding demo | A parent project injects a custom module and builds its own engine binary |
