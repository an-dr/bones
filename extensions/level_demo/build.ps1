#!/usr/bin/env pwsh
# Builds the level_demo extension into a WASM component, builds the bones
# engine, and assembles a runnable dist/level_demo/ next to it. Run with:
# pwsh build.ps1
...
Write-Host "Built: target/wasm32-wasip2/release/level_demo.wasm"
...
if (-not $dist.StartsWith($demoRoot + [IO.Path]::DirectorySeparatorChar)) {
    throw "Refusing to replace dist outside the level_demo directory"
}
...
Copy-Item "$PSScriptRoot/target/wasm32-wasip2/release/level_demo.wasm" "$dist/extensions/level_demo.wasm"

Write-Host ""
Write-Host "Packaged: $dist/$exeName (extensions/level_demo.wasm alongside it)"
