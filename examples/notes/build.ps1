...
# engine, and assembles a runnable dist/notes/ next to it. Run with:
# pwsh build.ps1
...
Write-Host "Built: target/wasm32-wasip2/release/notes.wasm"
...
if (-not $dist.StartsWith($demoRoot + [IO.Path]::DirectorySeparatorChar)) {
    throw "Refusing to replace dist outside the notes directory"
}
...
Copy-Item "$PSScriptRoot/target/wasm32-wasip2/release/notes.wasm" "$dist/extensions/notes.wasm"

Write-Host ""
Write-Host "Packaged: $dist/$exeName (extensions/notes.wasm alongside it)"
