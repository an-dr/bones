...
# engine, and assembles a runnable dist/keyecho/ next to it. Run with:
# pwsh build.ps1
...
if (-not $dist.StartsWith($demoRoot + [IO.Path]::DirectorySeparatorChar)) {
    throw "Refusing to replace dist outside the keyecho directory"
}
...
Copy-Item "$PSScriptRoot/target/wasm32-wasip2/release/keyecho.wasm" "$dist/extensions/keyecho.wasm"

Write-Host ""
Write-Host "Packaged: $dist/$exeName (extensions/keyecho.wasm alongside it)"
