#!/usr/bin/env pwsh
# Builds the persistence_demo extension into a WASM component, builds the
# bones engine, and assembles a runnable dist/persistence_demo/ next to
# it. Run with: pwsh build.ps1
...
Write-Host "Built: target/wasm32-wasip2/release/persistence_demo.wasm"
...
if (-not $dist.StartsWith($demoRoot + [IO.Path]::DirectorySeparatorChar)) {
    throw "Refusing to replace dist outside the persistence_demo directory"
}
...
Copy-Item "$PSScriptRoot/target/wasm32-wasip2/release/persistence_demo.wasm" "$dist/extensions/persistence_demo.wasm"

Write-Host ""
Write-Host "Packaged: $dist/$exeName (extensions/persistence_demo.wasm alongside it)"
