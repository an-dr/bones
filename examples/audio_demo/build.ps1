#!/usr/bin/env pwsh
# Builds the audio_demo extension into a WASM component, builds the bones
# engine, and assembles a runnable dist/audio_demo/ next to it. Run with:
# pwsh build.ps1
...
Write-Host "Built: target/wasm32-wasip2/release/audio_demo.wasm"
...
if (-not $dist.StartsWith($demoRoot + [IO.Path]::DirectorySeparatorChar)) {
    throw "Refusing to replace dist outside the audio_demo directory"
}
...
Copy-Item "$PSScriptRoot/target/wasm32-wasip2/release/audio_demo.wasm" "$dist/extensions/audio_demo.wasm"

Write-Host ""
Write-Host "Packaged: $dist/$exeName (extensions/audio_demo.wasm and bones.toml alongside it)"
