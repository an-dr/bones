#!/usr/bin/env pwsh
# Builds the sprite-demo extension into a WASM component, builds the
# bones engine, and assembles a runnable dist/sprite_demo/ next to it.
...
Write-Host "Built: target/wasm32-wasip2/release/sprite_demo.wasm"
...
if (-not $dist.StartsWith($demoRoot + [IO.Path]::DirectorySeparatorChar)) {
    throw "Refusing to replace dist outside the sprite_demo directory"
}
...
Copy-Item "$PSScriptRoot/target/wasm32-wasip2/release/sprite_demo.wasm" "$dist/extensions/sprite_demo.wasm"

Write-Host ""
Write-Host "Packaged: $dist/$exeName (extensions/sprite_demo.wasm alongside it)"
