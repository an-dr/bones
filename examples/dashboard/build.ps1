...
if (-not $dist.StartsWith($dashboardRoot + [IO.Path]::DirectorySeparatorChar)) {
    throw "Refusing to replace dist outside the dashboard directory"
}
...
Copy-Item "$PSScriptRoot/target/wasm32-wasip2/release/dashboard.wasm" "$dist/extensions/dashboard.wasm"
