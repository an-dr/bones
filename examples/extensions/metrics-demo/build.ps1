...
if (-not $dist.StartsWith($demoRoot + [IO.Path]::DirectorySeparatorChar)) {
    throw "Refusing to replace dist outside the metrics-demo directory"
}
...
Copy-Item "$PSScriptRoot/target/wasm32-wasip2/release/metrics_demo.wasm" "$dist/extensions/metrics_demo.wasm"

Write-Host ""
Write-Host "Packaged: $dist/$exeName (extensions/metrics_demo.wasm and bones.toml alongside it)"
