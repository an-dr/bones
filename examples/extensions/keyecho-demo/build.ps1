#!/usr/bin/env pwsh
# Builds the keyecho extension into a WASM component, builds the bones
# engine, and assembles a runnable dist/keyecho-demo/ next to it. Run with:
# pwsh build.ps1
...
Write-Host "Built: target/wasm32-wasip2/release/keyecho_demo.wasm"
...
if (-not $dist.StartsWith($demoRoot + [IO.Path]::DirectorySeparatorChar)) {
    throw "Refusing to replace dist outside the keyecho-demo directory"
}
...
Copy-Item "$PSScriptRoot/target/wasm32-wasip2/release/keyecho_demo.wasm" "$dist/extensions/keyecho_demo.wasm"

Write-Host ""
Write-Host "Packaged: $dist/$exeName (extensions/keyecho_demo.wasm alongside it)"
