#!/usr/bin/env pwsh
# Builds the game-core-demo extension into a WASM component, builds the
# bones engine, and assembles a runnable dist/game-core-demo/ next to it.
...
if (-not $dist.StartsWith($demoRoot + [IO.Path]::DirectorySeparatorChar)) {
    throw "Refusing to replace dist outside the game-core-demo directory"
}
