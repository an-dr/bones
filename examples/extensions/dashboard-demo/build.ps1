#!/usr/bin/env pwsh
# Builds the dashboard + metrics components and a web-enabled bones app,
# then assembles a directly runnable dist directory.
...
$metrics = Join-Path $PSScriptRoot "../metrics/Cargo.toml"
cargo build --manifest-path $metrics --target wasm32-wasip2 --release
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
...
$dashboardRoot = [IO.Path]::GetFullPath($PSScriptRoot)
if (-not $dist.StartsWith($dashboardRoot + [IO.Path]::DirectorySeparatorChar)) {
    throw "Refusing to replace dist outside the dashboard-demo directory"
}
...
Copy-Item "$PSScriptRoot/target/wasm32-wasip2/release/dashboard_demo.wasm" "$dist/extensions/dashboard_demo.wasm"
Copy-Item "$PSScriptRoot/../metrics/target/wasm32-wasip2/release/metrics.wasm" "$dist/extensions/metrics.wasm"

Write-Host ""
Write-Host "Dashboard ready: $dist/$exeName"
